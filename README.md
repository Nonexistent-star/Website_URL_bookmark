...
| 批量添加 | 一行一个，三种写法；解析后每行下拉选已有标签或输入新标签（自动新建） |
...
| 扩展日志 | 设置页彩色日志框 + 启用开关：监听扩展提交事件，按 信息(蓝)/成功(绿)/失败(红) 着色滚动；启用后同目录生成 `webjump_扩展日志.txt`，从端口开始监听记起，只记业务事件（识别信息、成功、失败），不记待机；2MB 上限自动轮转防日志过大 |
| 项目主页 | 设置页内置 GitHub 项目页入口，一键打开仓库 |
...
- **设置**：主题预设与逐块配色、默认打开方式、开机自启动、导出 CSV / JSON、扩展日志框与开关、GitHub 项目页入口
- **添加**：单个添加与批量添加
- **分类**：按标签聚合，点击筛选列表

![icon](web-kz/main.PNG)

## 快速开始

### 方式一：安装程序（推荐分发）

运行 `installer_out/WebJump-Setup-1.1.0.exe`：
...
└── installer_out/        # 安装包产物 WebJump-Setup-1.1.0.exe
```

## 常见问题

**Q：首次运行 exe 弹 SmartScreen 警告？**
A：仓库附带的是自签证书签名（`签名/` 目录，root.spc+root.pvk 合成 pfx 后用 signtool 签名）；自签证书未进入系统信任链时仍可能提示，选择"仍要运行"即可，或把 root.cer 导入受信任的根证书颁发机构。正式对外分发建议购买受信任 CA 的代码签名证书。
